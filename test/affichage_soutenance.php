<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Affichage</title>
    <style>
        h1{
            text-align : center ;
        }
table {
  font-family: arial, sans-serif;
  border-collapse: collapse;
  margin : 0 auto;
  width: 80%;
}

td, th {
  border: 1px solid #dddddd;
  text-align: left;
  padding: 8px;
}

tr:nth-child(even) {
  background-color: #dddddd;
}
</style>
</head>
<body>
    <?php require("connexion.php");
    $sql = "SELECT * FROM soutenir";
    $stmt = $bdd->query($sql);
    if ($stmt === false){
        die("error");
    }

    ?>
    <h1>Liste des Soutenances</h1>
    <table>
        <thead>
            <tr>
                <th>Matricule Etudiant</th>
                <th>Année Universitaire</th>
                <th>ID Organisme de l'etudiant</th>
                <th>Note de l'etudiant</th>
                <th>President de jury</th>
                <th>Examinateur</th>
                <th>Rapporteur interne</th>
                <th>Rapporteur externe</th>
            </tr>
        </thead>
        <tbody>
            <?php while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) : ?>
            <tr>
                <td><?php echo htmlspecialchars($row ['matricule']);?></td>
                <td><?php echo htmlspecialchars ($row['annee_univ']) ;?></td>
                <td><?php echo htmlspecialchars($row ['id_org']);?></td>
                <td><?php echo htmlspecialchars ($row['note']) ;?></td>
                <td><?php echo htmlspecialchars($row ['president']);?></td>
                <td><?php echo htmlspecialchars ($row['examinateur']) ;?></td>
                <td><?php echo htmlspecialchars($row ['rapporteur_int']);?></td>
                <td><?php echo htmlspecialchars ($row['rapporteur_ext']) ;?></td>
            </tr>
            <?php endwhile; ?>
        </tbody>
    </table>
</body>
</html>